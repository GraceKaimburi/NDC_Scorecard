...
export const SectorCapacityCards = ({ data }) => {
	const { implementationData, developmentData, currentSectorCategory } =
		useDashboardData();
	const sectors = [
				...
				: Object.keys(data).map((sectorKey) => {
						...
						return (
							<motion.div
								...
								initial={{ opacity: 0, y: 20 }}
								animate={{ opacity: 1, y: 0 }}
								className="bg-white  rounded-lg shadow-lg p-4 py-8"
								// layoutId=""
							>
								<div
								...
								<div className="space-y-2">
									<div className="flex">
										...
										>
											{sector?.[currentSectorCategory].score} (
											{sector?.[currentSectorCategory].label})
										</span>
									</div>
								</div>
							</motion.div>
